...
public abstract class Sprite extends VisibleComponent
   ...
   * If {@link #Bounce} is then called with that edge, the sprite will appear to bounce off
   ...
   */
  @SimpleEvent
  public void EdgeReachedOptions(Direction edge) {
   ...
   */
  @SimpleFunction
  public void BounceOptions(Direction edge) {
    MoveIntoBounds();
   ...
   * {@link #EdgeReached} event if the Sprite has reached the edge of the
   * Canvas.
   */
  protected void registerChange() {
    ...
    Direction edge = hitEdgeOptions();
    if (edge != null) {
      EdgeReachedOptions(edge);
    }
    canvas.registerChange(this);
  }
  ...
  protected int hitEdge(int canvasWidth, int canvasHeight) {
    Direction edge = hitEdgeOptions(canvasWidth, canvasHeight);
    if (edge == null) {
      return Component.DIRECTION_NONE;
    }
    return edge.getValue();
  ...
  protected Direction hitEdgeOptions() {
    if (!canvas.ready()) {
      return null;
    }
    return hitEdgeOptions(canvas.Width(), canvas.Height());
  }

  protected Direction hitEdgeOptions(int canvasWidth, int canvasHeight) {
